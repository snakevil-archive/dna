return function (DNA)
    ...
    DNA.triggers['dna.setup'] = function ()
        xpcall(function ()
            ...
            DNA.log(': module \'LuaSocket\' missing', nil, DNA.log.EMERGENCY, 1)
        end)
    end
    ...
    DNA.triggers['dna.server.setup.fail'] = function (context)
        DNA.log(': failed to bind \'' .. context.host .. '.' .. context.port .. '\' for ' .. context.reason, nil, DNA.log.EMERGENCY, 2)
    end
    ...
    DNA.triggers['dna.server.accept'] = function (request)
        ...
        DNA.log(request.host .. '.' .. request.port .. ': query \'' .. domain .. '\'', nil, DNA.log.NOTICE)
    end

    --- DNA.triggers['dna.agent.setup.fail']() - Handles 'dna.agent.setup.fail' event
    -- @param context Table of event context
    DNA.triggers['dna.agent.setup.fail'] = function (context)
        DNA.log('@DnaAgent-' .. context.agent.host .. '.' .. context.agent.port .. '#' .. context.agent.mode .. ': ' .. context.reason, nil, DNA.log.ALERT)
    end

    --- DNA.triggers['dna.agent.query']() - Handles 'dna.agent.query' event
    -- @param context Table of event context
    DNA.triggers['dna.agent.query'] = function (context)
        DNA.log('@DnaAgent-' .. context.agent.host .. '.' .. context.agent.port .. '#' .. context.agent.mode .. ': #' .. 1 + context.agent.counter .. ' send ' .. #context.query .. ' bytes', nil, DNA.log.INFO)
    end
    ...
    DNA.triggers['dna.agent.query.done'] = function (context)
        DNA.log('@DnaAgent-' .. context.agent.host .. '.' .. context.agent.port .. '#' .. context.agent.mode .. ': #' .. 1 + context.agent.counter .. ' receive ' .. #context.result .. ' bytes', nil, DNA.log.INFO)
    end
    ...
    DNA.triggers['dna.server.touch.done'] = function (response)
        DNA.log(response.host .. '.' .. response.port .. ': got ' .. #DNSHosts .. ' records', nil, DNA.log.NOTICE)
    end
