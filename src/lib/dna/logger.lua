...
return setmetatable({
    ...
    --- DnaLogger() - Prepares the logger
    -- @param path File to log
    -- @param level Which serverity and above should to log
    -- @return DnaLogger object
    __call = function (logger, path, level)
        if 'string' == type(path) and #path then
            DnaLogPath = path
        end
        if 'string' == type(level) and #level and logger.log[level:upper()] then
            DnaLogLevel = logger.log[level:upper()]
        end
        return logger
    end
})
